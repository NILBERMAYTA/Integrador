...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <a href="{{ auth()->user()->hasRole('policia') ? route('prestamos.index') : route('dashboard') }}" class="ms-2 me-5 flex items-center space-x-2 rtl:space-x-reverse lg:ms-0" wire:navigate>
                <x-app-logo />
            </a>
            ...
            <flux:navbar class="-mb-px max-lg:hidden">
                @role('policia')
                    @can('prestamos.view')
                        <flux:navbar.item icon="arrows-right-left" :href="route('prestamos.index')" :current="request()->routeIs('prestamos.*')" wire:navigate>
                            Mis Prestamos
                        </flux:navbar.item>
                    @endcan
                @else
        ...
        <flux:sidebar stashable sticky class="lg:hidden border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <a href="{{ auth()->user()->hasRole('policia') ? route('prestamos.index') : route('dashboard') }}" class="ms-1 flex items-center space-x-2 rtl:space-x-reverse" wire:navigate>
                <x-app-logo />
            </a>
