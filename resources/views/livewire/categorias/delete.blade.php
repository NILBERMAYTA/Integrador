<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)] opacity-70">
                Gestiona las categorías que han sido eliminadas del sistema
            </p>
        </div>
        <a 
    ...
    <div class="bg-[var(--color-surface)] dark:bg-[var(--color-surface-dark)] rounded-[var(--radius-radius)] shadow-sm border border-[var(--color-outline)] dark:border-[var(--color-outline-dark)] overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-[var(--color-outline)] dark:divide-[var(--color-outline-dark)] bg-[var(--color-surface)] dark:bg-[var(--color-surface-dark)]">
                    @forelse ($categoriasEliminadas as $cat)
                        <tr class="hover:bg-[var(--color-surface-alt)] dark:hover:bg-[var(--color-surface-dark-alt)] transition-colors opacity-75">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-medium text-[var(--color-on-surface-strong)] dark:text-[var(--color-on-surface-dark-strong)]">{{ $cat->id }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-semibold text-[var(--color-on-surface-strong)] dark:text-[var(--color-on-surface-dark-strong)]">{{ $cat->nombre }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)]">
                                {{ optional($cat->deleted_at)->diffForHumans() }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div x-data="{ openRestore: false, openDelete: false }" class="flex items-center gap-2">
                                    ...
                                    <x-form.confirm-modal
                                        ...
                                        icon="info"
                                        confirmText="Restaurar Categoría"
                                        cancelText="Cancelar"
                                        :persistent="false"
                                        maxWidth="lg"
                                        @confirm="$wire.restaurar({{ $cat->id }}); openRestore = false"
                                    >
                                        <p class="font-medium text-[var(--color-on-surface-strong)] dark:text-[var(--color-on-surface-dark-strong)] mb-2">
                                            ¿Desea restaurar la categoría <span class="font-semibold">{{ $cat->nombre }}</span>?
                                        </p>
                                        <p class="text-sm opacity-75 text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)]">
                                            Volverá a la lista de categorías activas con toda su información.
                                        </p>
                                    </x-form.confirm-modal>
                                    ...
                                    <x-form.confirm-modal
                                        ...
                                        :persistent="false"
                                        maxWidth="lg"
                                        @confirm="$wire.eliminarPermanentemente({{ $cat->id }}); openDelete = false"
                                    >
                                        <p class="font-medium text-[var(--color-on-surface-strong)] dark:text-[var(--color-on-surface-dark-strong)] mb-2">
                                            ⚠️ Esta acción es <span class="font-semibold">irreversible</span>.
                                        </p>
                                        <p class="text-sm opacity-75 text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)]">
                                            ¿Está seguro de eliminar definitivamente la categoría <span class="font-semibold">{{ $cat->nombre }}</span>?
                                        </p>
                                    </x-form.confirm-modal>
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-4 text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)] font-medium">No hay categorías eliminadas</p>
                                <p class="mt-1 text-sm text-[var(--color-on-surface)] dark:text-[var(--color-on-surface-dark)] opacity-60">Todas las categorías están activas</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($categoriasEliminadas->hasPages())
